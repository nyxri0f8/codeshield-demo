          ...
          <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid rgba(255, 255, 255, 0.08)' }}>
                ...
                <th style={{ padding: '12px 24px', fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>File</th>
                <th style={{ padding: '12px 24px', fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Line</th>
            ...
            <tbody>
              {endpoints.map((e) => {
                const match = e.reasoning?.match(/^\[(.*?)\]\s*(.*)$/);
                const filePath = match ? match[1] : 'unknown';
                const cleanReasoning = match ? match[2] : e.reasoning;
                return (
                  <tr key={e.id} style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.04)', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                    ...
                    {/* File */}
                    <td style={{ padding: '16px 24px', fontFamily: 'var(--font-mono)', color: 'var(--cyan-primary)' }}>{filePath}</td>
                    {/* Line */}
                    <td style={{ padding: '16px 24px', fontFamily: 'var(--font-mono)' }}>{e.line_number || 1}</td>
                    {/* Risk */}
                    ...
                    {/* Reasoning */}
                    <td style={{ padding: '16px 24px', color: 'var(--text-muted)' }}>{cleanReasoning}</td>
                  </tr>
                );
              })}
            </tbody>
